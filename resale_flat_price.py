...
def predict_price(year,town,flat_type,flr_area_sqm,flat_model,stry_start,stry_end,re_les_year,
    ...
    st.write(year,town,flat_type,flr_area_sqm,flat_model,stry_start,stry_end,re_les_year,
              re_les_month,les_coms_dt)
...
if select == "Home":
    ...
    st.write('''The majority of Singaporeans live in public housing provided by the Housing and Development Board.
    ...
    with col2:
        ...
        stry_start= st.number_input("Enter the Value of Storey Start ((Min: 4)")

        stry_end= st.number_input("Enter the Value of Storey End (Min: 10)") 
    ...
    #st.write("Original Floor Area (sqm):", flr_area_sqm)

    button= st.button("Predict the Price", use_container_width= True) 

    st.write(year, town, flat_type, flr_area_sqm, flat_model,
